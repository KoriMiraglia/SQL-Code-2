...
--Return the top 10 records, sorted high to low by seats. (this is repeated so I am doing the query below)
...
--List the distinct values of combinations for year, manufacturer,
--and model, sorted by manufacturer A-Z, then year oldest to newest, and then model A-Z.
SELECT DISTINCT year, manufacturer, model
FROM planes
ORDER BY manufacturer, year DESC, model;
